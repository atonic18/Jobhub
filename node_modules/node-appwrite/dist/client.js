...
const _Client = class _Client {
  ...
  async chunkedUpload(method, url, headers = {}, originalPayload = {}, onProgress) {
    const [fileParam, file] = Object.entries(originalPayload).find(
      ([_, value]) => value instanceof nodeFetchNativeWithAgent.File || value instanceof inputFile.InputFile
    ) ?? [];
    if (!file || !fileParam) {
      throw new Error("File not found in payload");
    }
    if (file instanceof inputFile.InputFile) {
      ...
      firstPayload2[fileParam] = new nodeFetchNativeWithAgent.File([firstChunk2], file.filename);
      ...
      const uploadChunk2 = async (chunk) => {
        ...
        chunkPayload[fileParam] = new nodeFetchNativeWithAgent.File([chunkBlob], file.filename);
    ...
    firstPayload[fileParam] = new nodeFetchNativeWithAgent.File([firstChunk], file.name);
    ...
    const uploadChunk = async (chunk) => {
      ...
      const chunkPayload = { ...originalPayload };
      chunkPayload[fileParam] = new nodeFetchNativeWithAgent.File([chunkBlob], file.name);
      const chunkResponse = await this.call(method, url, chunkHeaders, chunkPayload);
      if (failed) {
        return chunkResponse;
      }
